...
public class IcePetTest {
  private IcePet testPet;
  private IcePet anotherPet;
  ...
  @Before
  public void setup() {
    testPet = new IcePet("Bubbles", 1);
  }

  @Test
  public void pet_instantiatesCorrectly_true() {
    assertEquals(true, testPet instanceof IcePet);
  }

  @Test
  public void pet_instantiatesWithHalfPlayLevel() {
    assertEquals(testPet.getPlayLevel(), IcePet.MAX_PLAY_LEVEL/2);
  }

  @Test
  public void pet_instantiatesWithHalfSleepLevel() {
    assertEquals(testPet.getSleepLevel(), IcePet.MAX_SLEEP_LEVEL/2);
  }

  @Test
  public void pet_instantiatesWithHalfFoodLevel() {
    assertEquals(testPet.getFoodLevel(), IcePet.MAX_FOOD_LEVEL/2);
  }
  ...
  public void depleteLevels_reducesAllLevels() {
    ...
    assertEquals(testPet.getFoodLevel(), (IcePet.MAX_FOOD_LEVEL/2)-1);
    assertEquals(testPet.getSleepLevel(), (IcePet.MAX_SLEEP_LEVEL/2)-1);
    assertEquals(testPet.getPlayLevel(), (IcePet.MAX_PLAY_LEVEL/2)-1);
  }

  @Test
  public void sleep_increasesPetSleepLevel() {
    testPet.sleep();
    assertTrue(testPet.getSleepLevel()>(IcePet.MAX_SLEEP_LEVEL/2));
  }

  @Test
  public void feed_increasesPetFoodLevel() {
    testPet.feed();
    assertTrue(testPet.getFoodLevel()>(IcePet.MAX_FOOD_LEVEL/2));
  }

  @Test
  public void play_increasesPetPlayLevel() {
    testPet.play();
    assertTrue(testPet.getPlayLevel()>(IcePet.MAX_PLAY_LEVEL/2));
  }

  @Test
  public void play_recordsTimeLastPlayedInDatabase() {
    testPet.play();
    Timestamp lastPlayed = IcePet.find(testPet.getId()).getLastPlayed();
  ...
  public void pet_foodLevelCannotGoBeyondMaxValue(){
    for(int i = IcePet.MIN_ALL_LEVELS; i <= (IcePet.MAX_FOOD_LEVEL + 2); i++){
      try {
        testPet.feed();
      } catch (UnsupportedOperationException exception) { }
    }
    assertTrue(testPet.getFoodLevel() <= IcePet.MAX_FOOD_LEVEL);
  }

  @Test
  public void feed_recordsTimeLastAteInDatabase() {
    testPet.feed();
    Timestamp lastAte = IcePet.find(testPet.getId()).getLastAte();
    Timestamp rightNow = new Timestamp(new Date().getTime());
  ...
  public void pet_sleepLevelCannotGoBeyondMaxValue(){
    for(int i = IcePet.MIN_ALL_LEVELS; i <= (IcePet.MAX_SLEEP_LEVEL + 2); i++){
      try {
        testPet.sleep();
      } catch (UnsupportedOperationException exception) { }
    }
    assertTrue(testPet.getSleepLevel() <= IcePet.MAX_SLEEP_LEVEL);
  }

  @Test
  public void sleep_recordsTimeLastSleptInDatabase() {
    testPet.sleep();
    Timestamp lastSlept = IcePet.find(testPet.getId()).getLastSlept();
    Timestamp rightNow = new Timestamp(new Date().getTime());
  ...
  public void pet_playLevelCannotGoBeyondMaxValue(){
    for(int i = IcePet.MIN_ALL_LEVELS; i <= (IcePet.MAX_PLAY_LEVEL + 2); i++){
      try {
        testPet.play();
      } catch (UnsupportedOperationException exception) { }
    }
    assertTrue(testPet.getPlayLevel() <= IcePet.MAX_PLAY_LEVEL);
  }

  @Test
  public void isAlive_recognizeWhenPetDies_true() {
    anotherPet = new IcePet("Spud", 1);
    for(int i = IcePet.MIN_ALL_LEVELS; i<= IcePet.MAX_FOOD_LEVEL; i++) {
  ...
  public void save_assignsIdToPet() {
    IcePet savedPet = IcePet.all().get(0);
    assertEquals(savedPet.getId(), testPet.getId());
  }

  @Test
  public void save_recordsTimeOfCreationInDatabase() {
    Timestamp savedPetBirthday = IcePet.find(testPet.getId()).getBirthday();
    Timestamp rightNow = new Timestamp(new Date().getTime());
  ...
  public void equals_returnsTrueIfNameAndPLayerIDAreSame_true() {
    anotherPet = new IcePet("Bubbles", 1);
    assertTrue(anotherPet.equals(testPet));
  }

  @Test
  public void save_returnsTrueIfNamesAreTheSame() {
    assertTrue(IcePet.all().get(0).equals(testPet));
  }

  @Test
  public void save_savesPlayerIdToDB_true() {
    Player player = new Player("Henry", "henry@henry.com");
    anotherPet = new IcePet("Spud", player.getId());
    IcePet savedPet = IcePet.find(anotherPet.getId());
  ...
  public void find_returnsPetWithSameId_otherPet() {
    anotherPet = new IcePet("Spud", 1);
    assertTrue(IcePet.find(anotherPet.getId()).equals(anotherPet));
  }
  ...
  public void all_returnsAllInstancesOfPet_true() {
    anotherPet = new IcePet("Spud", 1);
    assertTrue(IcePet.all().get(0).equals(testPet));
    assertTrue(IcePet.all().get(1).equals(anotherPet));
  }
