...
public class NormalPetTest {
  private NormalPet testPet;
  private NormalPet anotherPet;
  ...
  @Before
  public void setup() {
    testPet = new NormalPet("Bubbles", 1);
  }

  @Test
  public void pet_instantiatesCorrectly_true() {
    assertEquals(true, testPet instanceof NormalPet);
  }

  @Test
  public void pet_instantiatesWithHalfPlayLevel() {
    assertEquals(testPet.getPlayLevel(), NormalPet.MAX_PLAY_LEVEL/2);
  }

  @Test
  public void pet_instantiatesWithHalfSleepLevel() {
    assertEquals(testPet.getSleepLevel(), NormalPet.MAX_SLEEP_LEVEL/2);
  }

  @Test
  public void pet_instantiatesWithHalfFoodLevel() {
    assertEquals(testPet.getFoodLevel(), NormalPet.MAX_FOOD_LEVEL/2);
  }
  ...
  public void depleteLevels_reducesAllLevels() {
    ...
    assertEquals(testPet.getFoodLevel(), (NormalPet.MAX_FOOD_LEVEL/2)-1);
    assertEquals(testPet.getSleepLevel(), (NormalPet.MAX_SLEEP_LEVEL/2)-1);
    assertEquals(testPet.getPlayLevel(), (NormalPet.MAX_PLAY_LEVEL/2)-1);
  }

  @Test
  public void sleep_increasesPetSleepLevel() {
    testPet.sleep();
    assertTrue(testPet.getSleepLevel()>(NormalPet.MAX_SLEEP_LEVEL/2));
  }

  @Test
  public void feed_increasesPetFoodLevel() {
    testPet.feed();
    assertTrue(testPet.getFoodLevel()>(NormalPet.MAX_FOOD_LEVEL/2));
  }

  @Test
  public void play_increasesPetPlayLevel() {
    testPet.play();
    assertTrue(testPet.getPlayLevel()>(NormalPet.MAX_PLAY_LEVEL/2));
  }
  ...
  public void pet_foodLevelCannotGoBeyondMaxValue(){
    for(int i = NormalPet.MIN_ALL_LEVELS; i <= (NormalPet.MAX_FOOD_LEVEL + 2); i++){
      try {
        testPet.feed();
      } catch (UnsupportedOperationException exception) { }
    }
    assertTrue(testPet.getFoodLevel() <= NormalPet.MAX_FOOD_LEVEL);
  }

  @Test
  public void feed_recordsTimeLastAteInDatabase() {
    testPet.feed();
    Timestamp lastAte = NormalPet.find(testPet.getId()).getLastAte();
    Timestamp rightNow = new Timestamp(new Date().getTime());
  ...
  public void pet_sleepLevelCannotGoBeyondMaxValue(){
    for(int i = NormalPet.MIN_ALL_LEVELS; i <= (NormalPet.MAX_SLEEP_LEVEL + 2); i++){
      try {
        testPet.sleep();
      } catch (UnsupportedOperationException exception) { }
    }
    assertTrue(testPet.getSleepLevel() <= NormalPet.MAX_SLEEP_LEVEL);
  }

  @Test
  public void sleep_recordsTimeLastSleptInDatabase() {
    testPet.sleep();
    Timestamp lastSlept = NormalPet.find(testPet.getId()).getLastSlept();
    Timestamp rightNow = new Timestamp(new Date().getTime());
  ...
  public void pet_playLevelCannotGoBeyondMaxValue(){
    for(int i = NormalPet.MIN_ALL_LEVELS; i <= (NormalPet.MAX_PLAY_LEVEL + 2); i++){
      try {
        testPet.play();
      } catch (UnsupportedOperationException exception) { }
    }
    assertTrue(testPet.getPlayLevel() <= NormalPet.MAX_PLAY_LEVEL);
  }

  @Test
  public void isAlive_recognizeWhenPetDies_true() {
    anotherPet = new NormalPet("Spud", 1);
    for(int i = NormalPet.MIN_ALL_LEVELS; i<= NormalPet.MAX_FOOD_LEVEL; i++) {
  ...
  public void save_assignsIdToPet() {
    NormalPet savedPet = NormalPet.all().get(0);
    assertEquals(savedPet.getId(), testPet.getId());
  }

  @Test
  public void save_recordsTimeOfCreationInDatabase() {
    Timestamp savedPetBirthday = NormalPet.find(testPet.getId()).getBirthday();
    Timestamp rightNow = new Timestamp(new Date().getTime());
  ...
  public void equals_returnsTrueIfNameAndPLayerIDAreSame_true() {
    anotherPet = new NormalPet("Bubbles", 1);
    assertTrue(anotherPet.equals(testPet));
  }

  @Test
  public void save_returnsTrueIfNamesAreTheSame() {
    assertTrue(NormalPet.all().get(0).equals(testPet));
  }

  @Test
  public void save_savesPlayerIdToDB_true() {
    Player player = new Player("Henry", "henry@henry.com");
    anotherPet = new NormalPet("Spud", player.getId());
    NormalPet savedPet = Pet.find(anotherPet.getId());
  ...
  public void find_returnsPetWithSameId_otherPet() {
    anotherPet = new NormalPet("Spud", 1);
    assertTrue(NormalPet.find(anotherPet.getId()).equals(anotherPet));
  }
  ...
  public void all_returnsAllInstancesOfPet_true() {
    anotherPet = new NormalPet("Spud", 1);
    assertTrue(NormalPet.all().get(0).equals(testPet));
    assertTrue(NormalPet.all().get(1).equals(anotherPet));
  }
