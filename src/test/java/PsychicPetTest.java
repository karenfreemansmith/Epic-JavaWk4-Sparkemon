...
public class PsychicPetTest {
  private PsychicPet testPet;
  private PsychicPet anotherPet;
  ...
  @Before
  public void setup() {
    testPet = new PsychicPet("Bubbles", 1);
  }

  @Test
  public void pet_instantiatesCorrectly_true() {
    assertEquals(true, testPet instanceof PsychicPet);
  }

  @Test
  public void pet_instantiatesWithHalfPlayLevel() {
    assertEquals(testPet.getPlayLevel(), PsychicPet.MAX_PLAY_LEVEL/2);
  }

  @Test
  public void pet_instantiatesWithHalfSleepLevel() {
    assertEquals(testPet.getSleepLevel(), PsychicPet.MAX_SLEEP_LEVEL/2);
  }

  @Test
  public void pet_instantiatesWithHalfFoodLevel() {
    assertEquals(testPet.getFoodLevel(), PsychicPet.MAX_FOOD_LEVEL/2);
  }
  ...
  public void depleteLevels_reducesAllLevels() {
    ...
    assertEquals(testPet.getFoodLevel(), (PsychicPet.MAX_FOOD_LEVEL/2)-1);
    assertEquals(testPet.getSleepLevel(), (PsychicPet.MAX_SLEEP_LEVEL/2)-1);
    assertEquals(testPet.getPlayLevel(), (PsychicPet.MAX_PLAY_LEVEL/2)-1);
  }

  @Test
  public void sleep_increasesPetSleepLevel() {
    testPet.sleep();
    assertTrue(testPet.getSleepLevel()>(PsychicPet.MAX_SLEEP_LEVEL/2));
  }

  @Test
  public void feed_increasesPetFoodLevel() {
    testPet.feed();
    assertTrue(testPet.getFoodLevel()>(PsychicPet.MAX_FOOD_LEVEL/2));
  }

  @Test
  public void play_increasesPetPlayLevel() {
    testPet.play();
    assertTrue(testPet.getPlayLevel()>(PsychicPet.MAX_PLAY_LEVEL/2));
  }

  @Test
  public void play_recordsTimeLastPlayedInDatabase() {
    testPet.play();
    Timestamp lastPlayed = PsychicPet.find(testPet.getId()).getLastPlayed();
  ...
  public void pet_foodLevelCannotGoBeyondMaxValue(){
    for(int i = PsychicPet.MIN_ALL_LEVELS; i <= (PsychicPet.MAX_FOOD_LEVEL + 2); i++){
      try {
        testPet.feed();
      } catch (UnsupportedOperationException exception) { }
    }
    assertTrue(testPet.getFoodLevel() <= PsychicPet.MAX_FOOD_LEVEL);
  }

  @Test
  public void feed_recordsTimeLastAteInDatabase() {
    testPet.feed();
    Timestamp lastAte = PsychicPet.find(testPet.getId()).getLastAte();
    Timestamp rightNow = new Timestamp(new Date().getTime());
  ...
  public void pet_sleepLevelCannotGoBeyondMaxValue(){
    for(int i = PsychicPet.MIN_ALL_LEVELS; i <= (PsychicPet.MAX_SLEEP_LEVEL + 2); i++){
      try {
        testPet.sleep();
      } catch (UnsupportedOperationException exception) { }
    }
    assertTrue(testPet.getSleepLevel() <= PsychicPet.MAX_SLEEP_LEVEL);
  }

  @Test
  public void sleep_recordsTimeLastSleptInDatabase() {
    testPet.sleep();
    Timestamp lastSlept = PsychicPet.find(testPet.getId()).getLastSlept();
    Timestamp rightNow = new Timestamp(new Date().getTime());
  ...
  public void pet_playLevelCannotGoBeyondMaxValue(){
    for(int i = PsychicPet.MIN_ALL_LEVELS; i <= (PsychicPet.MAX_PLAY_LEVEL + 2); i++){
      try {
        testPet.play();
      } catch (UnsupportedOperationException exception) { }
    }
    assertTrue(testPet.getPlayLevel() <= PsychicPet.MAX_PLAY_LEVEL);
  }

  @Test
  public void isAlive_recognizeWhenPetDies_true() {
    anotherPet = new PsychicPet("Spud", 1);
    for(int i = PsychicPet.MIN_ALL_LEVELS; i<= PsychicPet.MAX_FOOD_LEVEL; i++) {
  ...
  public void save_assignsIdToPet() {
    PsychicPet savedPet = PsychicPet.all().get(0);
    assertEquals(savedPet.getId(), testPet.getId());
  }

  @Test
  public void save_recordsTimeOfCreationInDatabase() {
    Timestamp savedPetBirthday = PsychicPet.find(testPet.getId()).getBirthday();
    Timestamp rightNow = new Timestamp(new Date().getTime());
  ...
  public void equals_returnsTrueIfNameAndPLayerIDAreSame_true() {
    anotherPet = new PsychicPet("Bubbles", 1);
    assertTrue(anotherPet.equals(testPet));
  }

  @Test
  public void save_returnsTrueIfNamesAreTheSame() {
    assertTrue(PsychicPet.all().get(0).equals(testPet));
  }

  @Test
  public void save_savesPlayerIdToDB_true() {
    Player player = new Player("Henry", "henry@henry.com");
    anotherPet = new PsychicPet("Spud", player.getId());
    PsychicPet savedPet = PsychicPet.find(anotherPet.getId());
  ...
  public void find_returnsPetWithSameId_otherPet() {
    anotherPet = new PsychicPet("Spud", 1);
    assertTrue(PsychicPet.find(anotherPet.getId()).equals(anotherPet));
  }
  ...
  public void all_returnsAllInstancesOfPet_true() {
    anotherPet = new PsychicPet("Spud", 1);
    assertTrue(PsychicPet.all().get(0).equals(testPet));
    assertTrue(PsychicPet.all().get(1).equals(anotherPet));
  }
