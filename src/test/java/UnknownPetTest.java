...
public class UnknownPetTest {
  private UnknownPet testPet;
  private UnknownPet anotherPet;
  ...
  @Before
  public void setup() {
    testPet = new UnknownPet("Bubbles", 1);
  }

  @Test
  public void pet_instantiatesCorrectly_true() {
    assertEquals(true, testPet instanceof UnknownPet);
  }

  @Test
  public void pet_instantiatesWithHalfPlayLevel() {
    assertEquals(testPet.getPlayLevel(), UnknownPet.MAX_PLAY_LEVEL/2);
  }

  @Test
  public void pet_instantiatesWithHalfSleepLevel() {
    assertEquals(testPet.getSleepLevel(), UnknownPet.MAX_SLEEP_LEVEL/2);
  }

  @Test
  public void pet_instantiatesWithHalfFoodLevel() {
    assertEquals(testPet.getFoodLevel(), UnknownPet.MAX_FOOD_LEVEL/2);
  }
  ...
  public void depleteLevels_reducesAllLevels() {
    ...
    assertEquals(testPet.getFoodLevel(), (UnknownPet.MAX_FOOD_LEVEL/2)-1);
    assertEquals(testPet.getSleepLevel(), (UnknownPet.MAX_SLEEP_LEVEL/2)-1);
    assertEquals(testPet.getPlayLevel(), (UnknownPet.MAX_PLAY_LEVEL/2)-1);
  }

  @Test
  public void sleep_increasesPetSleepLevel() {
    testPet.sleep();
    assertTrue(testPet.getSleepLevel()>(UnknownPet.MAX_SLEEP_LEVEL/2));
  }

  @Test
  public void feed_increasesPetFoodLevel() {
    testPet.feed();
    assertTrue(testPet.getFoodLevel()>(UnknownPet.MAX_FOOD_LEVEL/2));
  }

  @Test
  public void play_increasesPetPlayLevel() {
    testPet.play();
    assertTrue(testPet.getPlayLevel()>(UnknownPet.MAX_PLAY_LEVEL/2));
  }

  @Test
  public void play_recordsTimeLastPlayedInDatabase() {
    testPet.play();
    Timestamp lastPlayed = UnknownPet.find(testPet.getId()).getLastPlayed();
  ...
  public void pet_foodLevelCannotGoBeyondMaxValue(){
    for(int i = UnknownPet.MIN_ALL_LEVELS; i <= (UnknownPet.MAX_FOOD_LEVEL + 2); i++){
      try {
        testPet.feed();
      } catch (UnsupportedOperationException exception) { }
    }
    assertTrue(testPet.getFoodLevel() <= UnknownPet.MAX_FOOD_LEVEL);
  }

  @Test
  public void feed_recordsTimeLastAteInDatabase() {
    testPet.feed();
    Timestamp lastAte = UnknownPet.find(testPet.getId()).getLastAte();
    Timestamp rightNow = new Timestamp(new Date().getTime());
  ...
  public void pet_sleepLevelCannotGoBeyondMaxValue(){
    for(int i = UnknownPet.MIN_ALL_LEVELS; i <= (UnknownPet.MAX_SLEEP_LEVEL + 2); i++){
      try {
        testPet.sleep();
      } catch (UnsupportedOperationException exception) { }
    }
    assertTrue(testPet.getSleepLevel() <= UnknownPet.MAX_SLEEP_LEVEL);
  }

  @Test
  public void sleep_recordsTimeLastSleptInDatabase() {
    testPet.sleep();
    Timestamp lastSlept = UnknownPet.find(testPet.getId()).getLastSlept();
    Timestamp rightNow = new Timestamp(new Date().getTime());
  ...
  public void pet_playLevelCannotGoBeyondMaxValue(){
    for(int i = UnknownPet.MIN_ALL_LEVELS; i <= (UnknownPet.MAX_PLAY_LEVEL + 2); i++){
      try {
        testPet.play();
      } catch (UnsupportedOperationException exception) { }
    }
    assertTrue(testPet.getPlayLevel() <= UnknownPet.MAX_PLAY_LEVEL);
  }

  @Test
  public void isAlive_recognizeWhenPetDies_true() {
    anotherPet = new UnknownPet("Spud", 1);
    for(int i = UnknownPet.MIN_ALL_LEVELS; i<= UnknownPet.MAX_FOOD_LEVEL; i++) {
  ...
  public void save_assignsIdToPet() {
    UnknownPet savedPet = UnknownPet.all().get(0);
    assertEquals(savedPet.getId(), testPet.getId());
  }

  @Test
  public void save_recordsTimeOfCreationInDatabase() {
    Timestamp savedPetBirthday = UnknownPet.find(testPet.getId()).getBirthday();
    Timestamp rightNow = new Timestamp(new Date().getTime());
  ...
  public void equals_returnsTrueIfNameAndPLayerIDAreSame_true() {
    anotherPet = new UnknownPet("Bubbles", 1);
    assertTrue(anotherPet.equals(testPet));
  }

  @Test
  public void save_returnsTrueIfNamesAreTheSame() {
    assertTrue(UnknownPet.all().get(0).equals(testPet));
  }

  @Test
  public void save_savesPlayerIdToDB_true() {
    Player player = new Player("Henry", "henry@henry.com");
    anotherPet = new UnknownPet("Spud", player.getId());
    UnknownPet savedPet = UnknownPet.find(anotherPet.getId());
  ...
  public void find_returnsPetWithSameId_otherPet() {
    anotherPet = new UnknownPet("Spud", 1);
    assertTrue(UnknownPet.find(anotherPet.getId()).equals(anotherPet));
  }
  ...
  public void all_returnsAllInstancesOfPet_true() {
    anotherPet = new UnknownPet("Spud", 1);
    assertTrue(UnknownPet.all().get(0).equals(testPet));
    assertTrue(UnknownPet.all().get(1).equals(anotherPet));
  }
