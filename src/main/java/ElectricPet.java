...
public class ElectricPet extends Pet implements DatabaseManagement {
  ...
  public static List<ElectricPet> all() {
    String sql = "SELECT * FROM pets WHERE type=4";
    try(Connection cn = DB.sql2o.open()) {
      return cn.createQuery(sql).throwOnMappingFailure(false).executeAndFetch(ElectricPet.class);
    }
  }
